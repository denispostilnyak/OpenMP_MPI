...
using namespace std;
const double RandomDataMultiplier = 10000.0;
...
void ProcessInitialization(double*& pData, int& DataSize, double
	*& pProcData, int& BlockSize) {
	...
	// Broadcasting the data size. Sharing input data between processes
...
void DataDistribution(double* pData, int DataSize, double* pProcData, int
	...
	//Disturbing data between processes
...
void DataCollection(double* pData, int DataSize, double* pProcData, int
	...
	//Collect data from all processes into one array
...
void ParallelBubble(double* pProcData, int BlockSize) {
	...
	int Offset;
	//On odd iteration odd process number should replace with neighbours at right and even process number should replace with neighbour at left
	//On even iteration odd precess number should replace with neighbour at left and even process number should replace with neighbour at right
	...
	for (int i = 0; i < ProcNum; i++) {
		...
		MPI_Status status;
		int DualBlockSize;
		//Send and receive data for exchange operation
...
void ParallelPrintData(double* pProcData, int BlockSize) {
	...
	for (int i = 0; i < ProcNum; i++) {
		...
		//block all calls till the all precesses call this precedure.
		MPI_Barrier(MPI_COMM_WORLD);
	}
}
...
void TestResult(double* pData, double* pSerialData, int DataSize) {
	...
	if (ProcRank == 0) {
		...
		bool isIdentical = CompareData(pData, pSerialData, DataSize);
